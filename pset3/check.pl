...
sub run_qitem ($) {
    ...
    $qitem->{"count"} += 1;
    $qitem->{"errors"} += 1 if exists($t->{"error"});
...
sub run ($) {
    ...
    for (my $qpos = 0; $qpos < @workq; $qpos += 1) {
        ...
        $tcompar->{"content_check"} = $qitem->{"outfiles"}
            if !$NOYOURCODE && !$NOSTDIO && !$qitem->{"no_content_check"} && $sequentially;
        $tcompar->{"md5sum_check"} = $t->{"md5sum"}
            if !$NOYOURCODE && $NOSTDIO && !$qitem->{"no_content_check"} && exists($t->{"md5sum"});
        ...
        if ($stdiot && $tt && $tt->{"time"} && !defined($tt->{"error"})
...
enqueue(9,
    "./gather61 -b 512 -o files/out.bin files/binary1meg.bin files/text1meg.txt",
